...
def SHIFT_THEN_INTERSECT(
    ...
    SHIFT_DNA_Protein(
        INIT_LOCS["ChIP"],
        f"{OUTDIR['ChIP']}/{PROT}.ChIP.{ORG}.bed",
        SHIFTS_DICT["ChIP"],
        chip_TADmap,
    )
    ...
    chromHmm_annot_cmd = (
        f"{intersectBed} -a {TMP_dir}/SIM_TRIADS/{fname}.tri.bed -b {chromhmm_loc} -wb"
    )
    ...
    with open(f"{TMP_dir}/SIM_TRIADS/{fname}.tri.bed", "w") as triad_fn:
        uniq_filt = sp.Popen(
            sh.split(uniq_filt_cmd), stdin=awk_filt.stdout, stdout=triad_fn
        )
    awk_filt.stdout.close()

    # CHECK ANNOTATION IF NEEDED
    if ANNOT_FLAG:
        chrom_hmm_pro = sp.Popen(
            sh.split(chromHmm_annot_cmd), stdout=sp.PIPE, text=True
        )
        cut_sort_proc = sp.Popen(
            cut_sort_cmd,
            shell=True,
            stdin=chrom_hmm_pro.stdout,
            stdout=sp.PIPE,
            text=True,
        )

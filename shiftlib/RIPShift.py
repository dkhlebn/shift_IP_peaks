...
def SHIFT_RNA_Protein(PEAKFILE_PATH, OUTFILE_PATH, SHIFT_DICT, CHR_LIST):
    ...
    def shift_rip_peak(shifts_dict, peak, limits_dct):
        ...
        end_shift = start_init + shift

        start_shift = start_shift + ft_len * (start_shift < dw_limit) - ft_len * (start_shift > up_limit)
        end_shift = end_shift + ft_len * (end_shift < dw_limit) - ft_len * (end_shift > up_limit)

        if start_shift > end_shift:
            peak_fromStart = (0, peak[1].copy())
            peak_fromStart[1]["start"] = int(start_shift)
            peak_fromStart[1]["end"] = limit

            peak_fromEnd = (0, peak[1].copy())
            peak_fromEnd[1]["start"] = 0
            peak_fromStart[1]["end"] = int(end_shift)
    ...
    for row in peaks.iterrows():
        shifted = shift_rip_peak(SHIFTS_DICT, row, minmax_chr)
        for el in shifted:
            data.append(el)
